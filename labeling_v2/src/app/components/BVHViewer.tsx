...
export function BVHViewer({ currentFrame, totalFrames, slices, showAnnotatedOverlay }: BVHViewerProps) {
  ...
  useEffect(() => {
    ...
    // Grid
    const gridHelper = new THREE.GridHelper(10, 20, 0x6b7280, 0x4b5563);
  ...
  return (
    <div className="w-full h-full bg-slate-950 rounded-lg overflow-hidden relative">
      ...
      {/* Overlay indicator */}
      <div className="absolute top-3 left-3 bg-slate-900/80 px-3 py-1.5 rounded text-xs text-slate-300">
        3D BVH 播放器
      </div>
    </div>
  );
